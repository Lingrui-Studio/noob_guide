...
你可以把 Git 理解成管理代码版本的工具，把 GitHub 理解成存放和协作开发这些代码的线上平台。

Git 与 GitHub 是单向依赖的：

- **Git** 即使不联网、不使用 GitHub，你也可以在本地创建仓库、提交修改和查看历史；许多企业内部也会用 GitLab 搭建自己的线上代码托管平台
- **GitHub** 在 **Git 的基础上**提供了远程备份、Issue、Pull Request、代码审查等功能，由微软公司管理
...
> [Learn Git Branching](https://learngitbranching.js.org/?locale=zh_CN) 也是一个非常好的图形化交互式教程，不过它主要关注 git branch，对新生来说可能就有点难了，推荐等有更多 git 经验之后再来学习。
...
- 为 Wiki 提 Issue：找出这个 Wiki 的 typo（错别字）、逻辑漏洞、或是经过你验证发现确实有错的内容，并通过 [Issues 界面](https://github.com/Lingrui-Studio/noob_guide/issues) 提交反馈
